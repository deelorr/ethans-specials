...
console.log(`Ethan Specials App v2.3 loaded: ${specials.length} specials`);
...
function openModal(r){ $('modalContent').innerHTML=`<h2>${val(r,'Special Name')}</h2><p class="muted">${val(r,'Week')} · ${val(r,'Date Range')} · ${val(r,'Year')}</p><div class="meta"><span class="tag">${val(r,'Category')}</span><span class="tag">${val(r,'Meal Type')}</span><span class="tag">Prep: ${val(r,'Prep Difficulty')}</span><span class="tag">Pickup: ${val(r,'Pickup Speed')}</span></div><h3>Ingredients</h3><p>${val(r,'Ingredients') || '—'}</p><h3>Build</h3><p><strong>Protein:</strong> ${val(r,'Protein') || '—'}<br><strong>Sauce:</strong> ${val(r,'Sauce') || '—'}<br><strong>Station:</strong> ${val(r,'Station') || '—'}</p><h3>Load</h3><p>Egg: ${val(r,'Egg Station Load') || '—'} · Flat Top: ${val(r,'Flat Top Load') || '—'} · Fryer: ${val(r,'Fryer Load') || '—'}</p><h3>Notes</h3><p>${val(r,'Notes') || 'No notes yet.'}</p>`; $('modal').classList.remove('hidden'); }
...
function statPanel(title, entries){return `<div class="panel"><h2>${title}</h2>${entries.slice(0,12).map(([k,v])=>`<div class="bar-row"><span>${k}</span><strong>${v}</strong></div>`).join('')}</div>`}
function init(){
 ...
 $('dashboardStats').innerHTML=`<div class="stat"><strong>${specials.length}</strong><span>Total specials</span></div><div class="stat"><strong>${uniq('Category').length}</strong><span>Categories</span></div><div class="stat"><strong>${uniq('Week').length}</strong><span>Weeks</span></div><div class="stat"><strong>${uniq('Primary Protein').length}</strong><span>Proteins</span></div>`;
 ...
 $('statsOutput').innerHTML=statPanel('Categories',countBy('Category'))+statPanel('Proteins',countBy('Primary Protein'))+statPanel('Station Load',countBy('Station'))+statPanel('Pickup Speed',countBy('Pickup Speed'));
